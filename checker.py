class Checker:
    ...
    def display(self):
        ...
        if self.color == "Black" and (self.can_move or self.can_jump):
            self.highlight()
    ...
    def mouseReleased(self):
        ...
        if self.can_jump and self.dragged:
            for new_x, new_y in self.valid_jump:
                if new_x * self.SQUARE_SIZE < mouseX \
                   ...
                   < new_y * self.SQUARE_SIZE + self.SQUARE_SIZE:
                    ...
                    self.board_x = self.HALF_SQAURE + new_x * self.SQUARE_SIZE
                    self.board_y = self.HALF_SQAURE + new_y * self.SQUARE_SIZE
                else:
                    self.board_x = self.HALF_SQAURE + self.x * self.SQUARE_SIZE
                    self.board_y = self.HALF_SQAURE + self.y * self.SQUARE_SIZE
                self.dragged = False

        if self.can_move and self.dragged:
